...
Vec4d calc_avg8(const VSet4i& lat, const _3DScalar4d& _3DVal_4d) {
	...
	for (int k = 0; k < 8; k++) {
        ...
        avg_elem_index_x=avg_lat[k].x-avg_vec_index_x*4; //index of the elem in 4-pair
			   //édï˚Ç»Ç¢ÇÃÇ≈ÇSÇ¬ÉoÉâÉoÉâÇ…ÇµÇƒéÊìæ
		...
		for (int i = calc_index_min / 4; i < calc_index_max / 4; i++) {
			...
			if (all_current_cells[i].hasState(DEAD)) {
				...
				for (int j = 0; j < C::max_cell_num / 4; j++) {
					if (all_current_cells[j].hasState(ALIVE)) { //Ç±ÇÍÇ≈ÇÕëSÇƒÇÃç◊ñEÇ…ëŒÇµÇƒåvéZÇ∑ÇÈ
						//Ç‚Ç¡ÇœÇËèdÇ¢Ç»ÇÁïœÇ¶ÇÈ
						//Pj-Pi
			...
			//êÊÇ…DEADÉ}ÉXÉNÇÇ©ÇØÇ∏Ç…neg_me_dead_val+react_alive_valÇåvéZÇµÇƒÇ©ÇÁÇ©ÇØÇΩÇŸÇ§Ç™ó«Ç¢?
			//->ïœÇÌÇÁÇ»Ç¢,ÇØÇ«å©Ç‚Ç∑Ç¢Ç©Ç‡

...
#define ANZ ((int)(LX / AREA_GRID_ORIGINAL))
#define N3 64
#define N2 400
...
if(index<ncell){
	...
	//int aix, aiy, aiz;
	const int aix = (int)((0.5*LX - p_diff_x(0.5f*LX, c.x)) / AREA_GRID);
	const int aiy = (int)((0.5*LY - p_diff_y(0.5f*LY, c.y)) / AREA_GRID);
	...
	*reinterpret_cast<unsigned int*>(&c.w)=
...
__global__ void connect_proc(int nmemb,CellManager_Device*const RESTRICT cmd,const float4*const RESTRICT area_info ){
	...
	//radius always R_max
	const CellIndex my_index = nmemb+blockIdx.x;
	if(threadIdx.x == 0){
		printf("tesuya222\n");
	...
	int cz = virtual_id;
	int cy = ((int)(cz/SEARCH_PAR_NUM));cz = cz%SEARCH_PAR_NUM;
	int cx = ((int)(cy/SEARCH_PAR_NUM));cy = cy%SEARCH_PAR_NUM;
	__syncthreads();
	if(virtual_id<=SEARCH_GRID_NUM){
		const int aix = (an.x - (SEARCH_GRID_DIM/2)+cx+ANX)%ANX;
		const int aiy = (an.y - (SEARCH_GRID_DIM/2)+cy+ANY)%ANY;
		const int aiz = (an.z - (SEARCH_GRID_DIM/2)+cz+ANZ)%ANZ;
		const int idx3d = midx<ANX,ANY,ANZ>(aix,aiy,aiz);
		const int store_count= GET_AREA_INFO_STORE_COUNT(&area_info[idx3d+0].w);
...
__global__ void complete_area_info(const unsigned int*const RESTRICT aindx_3d,float4*const RESTRICT area_info){
	...
	*reinterpret_cast<unsigned int*>(&area_info[idx*N3+0].w)
			=MERGE_AREA_INFO_STORE_COUNT(num,GET_AREA_INFO_IDX(w));
}
...
void connect_cell(CellManager* cm){
	...
	static device_alloc_ctor<float4> area_info(ANX*ANY*ANZ);

	aindx.set_zero();
	area_info.set_zero();
	printf("connect cell 1\n");
	init_grid<<<256,256>>>(cm->ncell_host,cm->current_pos_host(),cm->dev_ptr,aindx.ptr,area_info.ptr);
	cudaDeviceSynchronize();
	printf("connect cell 2\n");
	complete_area_info<<<dim3(ANX,ANY),ANZ>>>(aindx.ptr,area_info.ptr);
	cudaDeviceSynchronize();
	printf("connect cell 3\n");
	connect_proc<<<cm->ncell_host-cm->nmemb_host,SEARCH_THREAD_UPPER_LIMIT*SEARCH_PAR_NUM>>>(cm->ncell_host,cm->dev_ptr,area_info.ptr);
	cudaDeviceSynchronize();
	printf("connect cell 4\n");

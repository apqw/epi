...
__device__ inline float4 normal_with_len_inv(const float4 a, const float4 b){
	const float diffx = p_diff_x(a.x, b.x);
	const float diffy = p_diff_y(a.y, b.y);
	const float diffz = a.z - b.z;
	const float len_inv = rsqrtf(diffx*diffx + diffy*diffy + diffz*diffz);
	return make_float4(diffx*len_inv, diffy*len_inv, diffz*len_inv, len_inv);
}

template<unsigned int AXIS>
__device__ inline float bend_calc_1(const float dot_rme, const  float dot_l, const float dot_ll, const float4 n_r, const float4 n_rme, const  float4 n_l, const  float4 n_ll){
	return 0.0f;
}//dummy
...
template<>
__device__ inline float bend_calc_1<0>(const float dot_rme, const float dot_l, const  float dot_ll, const  float4 n_r, const float4 n_rme, const float4 n_l, const  float4 n_ll){
	return __bend_calc_m(x);
}

template<>
__device__ inline float bend_calc_1<1>(const float dot_rme, const float dot_l, const  float dot_ll, const  float4 n_r, const float4 n_rme, const float4 n_l, const  float4 n_ll){
	return __bend_calc_m(y);
}
template<>
__device__ inline float bend_calc_1<2>(const float dot_rme, const float dot_l, const  float dot_ll, const  float4 n_r, const float4 n_rme, const float4 n_l, const  float4 n_ll){
	return __bend_calc_m(z);
}

__device__ inline float4 bend_calc_main(const CellPos cme, const CellPos cu, const CellPos cl, const CellPos cb, const CellPos cr,
	...
	const float dot_ume = dot3(n_u, n_ume);
	const float dot_b = dot3(n_ume, n_b);
	const float dot_bb = dot3(n_b, n_bb);

	const float dot_rme = dot3(n_r, n_rme);
	const float dot_l = dot3(n_rme, n_l);
	const float dot_ll = dot3(n_l, n_ll);
	...
	return make_float4(ax, ay, az,0.f);
}
...
__global__ void memb_bend_apply(int nmemb,const CellPos*const RESTRICT cs, CellPos*const RESTRICT new_cs){
	...
	if (i < nmemb){
		...
		const float4 straight_a = bend_calc_main(cme, cu, cl, cb, cr, cuu, cll, cbb, crr);
		...
		const float4 diag_a = bend_calc_main(cme, cu, cl, cb, cr, cuu, cll, cbb, crr);
...
__device__ float CI_memb_to_memb(const CellPos c1, const CellPos c2) {



	//using namespace cont;
	const float rad_sum = R_memb+R_memb;
	const float rad_sum_sq = rad_sum*rad_sum;

	float cr_dist_sq;
	const float distSq = p_cell_dist_sq(c1, c2);
	if (distSq< (cr_dist_sq = rad_sum_sq*P_MEMB*P_MEMB)) {
		...
		const float inv_distlj = rsqrtf(distSq);
...
__device__ float CI_memb_to_memb_diag(int myidx,int pidx,const CellPos c1, const CellPos c2) {



	//using namespace cont;
	const float rad_sum = 2.0*R_memb;
	const float rad_sum_sq = 4.0*R_memb*R_memb;
...
__device__ inline float ljmain(const CellPos c1, const CellPos c2,const float rad1,const float rad2) {
...
__device__ inline float diff_wall(const CellPos c1,const float rad1){
...
__device__ inline float CI_other(const CellPos c1, const CellPos c2, float rad1, float rad2){
...
__device__ inline float ljmain_der_near(const CellPos c1, const CellPos c2) {
	/*
	‹——£‚Ì2æ‚¾‚¯‚Å‚Í•s‰Â
	*/
	const float dist = sqrtf(p_cell_dist_sq(c1, c2));
	const float dist2 = dist + delta_R;
	const float rad_sum = 2.0f*R_der;
	const float LJ6 = POW6(rad_sum) / POW6(dist2);
	//LJ6 = LJ6*LJ6*LJ6;
	return 4.0f*eps_m*LJ6*(LJ6 - 1.0f) / (dist*dist2) + para_ljp2;
}

__device__ inline bool is_near(const CellPos c1, const CellPos c2,const float rad1,const float rad2) {
	//double rad_sum = c1->radius + c2->radius;
	return p_cell_dist_sq(c1, c2) < (rad1 + rad2)*(rad1 + rad2);
}
__device__ inline float ljmain_der_far(const CellPos c1, const CellPos c2) {
	const float dist = sqrtf(p_cell_dist_sq(c1, c2));
	const float dist2 = dist + delta_R;
	const float rad_sum = 2.0f*R_der;
	const float LJ6 = POW6(rad_sum) / POW6(dist2);
...
__device__ float CI_der_and_der(const CellPos c1, const CellPos c2){
...
__device__ float4 der_interact_impl(const CellPos cme, const CellConnectionData*const conn_ptr, const CellPos*const RESTRICT cs, const CELL_STATE*const RESTRICT cstate){
	float4 vel = { 0.0f };
	...
	for (int j = 0; j < conn_ptr->connect_num; j++){
		...
		const float coef = opstate == DER ? CI_der_and_der(cme, opcs) : CI_other(cme, opcs, R_der, get_radius(opstate));
...
__device__ inline float adhesion(const CellPos c1,const CellPos c2,const float rad1,const float rad2, const float spring_const) {
	//using namespace cont;
	const float distlj = sqrtf(p_cell_dist_sq(c1, c2));
	const float rad_sum = rad1 + rad2;
	//double LJ2_m1;
...
__device__ inline float CI_fix_mu_and_fix_mu(const CellPos c1, const CellPos c2) {
	//pair‚©‚Ç‚¤‚©‚ÍŠO‚Å”»’è‚µ‚æ‚¤
...
//always fix -> memb
__device__ float CI_fix_and_memb(const CellPos c1, const CellPos c2) {
	if (is_near(c1, c2, R_max, R_memb)) {
		return ljmain(c1, c2, R_max, R_memb);
	}
	else {
		const float distlj = sqrtf(p_cell_dist_sq(c1, c2));
		const float LJ2 = distlj / (R_max+ R_memb);
		return -(Kspring / distlj)*(LJ2 - 1.0f);
	}
}

__device__ float CI_al_air_de_fix_mu_and_al_air_de(const CellPos c1, const CellPos c2, float agek1, float agek2) {
	if (is_near(c1, c2, R_max, R_max)) {
		return ljmain(c1, c2, R_max, R_max);
	}
	else {
		const float spf = agek1 > THRESH_SP && agek2 > THRESH_SP? K_TOTAL:K_DESMOSOME;
		return adhesion(c1, c2, R_max, R_max, spf);
	}
}

__device__ float4 fix_interact_impl(const CellPos cme, const CellConnectionData*const conn_ptr, const CellPos*const RESTRICT cs, CellPos*const RESTRICT new_cs,//rewrite cs
	const CELL_STATE*const RESTRICT cstate,const float*const agek_arr,const float my_agek,const CellIndex pair_idx,const CellIndex dermis_idx){
	float4 vel = { 0.0f };
	//cell_pos_set cme = cs[index];
	...
	for (int j = 0; j < conn_ptr->connect_num; j++){
		...
		const CellPos opcs = cs[opidx];
		const CELL_STATE opstate = cstate[opidx];
		const float4 vs = p_diff4(cme, opcs);
		float coef = 0.0f;
		switch (opstate){
...
//always mu->memb
__device__ float CI_mu_and_memb(const CellPos c1, const CellPos c2,const float spring_force_to_memb) {
...
__device__ float4 musume_interact_impl(const CellPos cme, const CellConnectionData*const conn_ptr, const CellPos*const RESTRICT cs, CellPos*const RESTRICT new_cs, //rewrite cs
	const CELL_STATE*const RESTRICT cstate, const float*const agek_arr, const int my_rest_div_time,const float my_agek, const CellIndex pair_idx, const CellIndex dermis_idx){
	float4 vel = { 0.0f };
	//cell_pos_set cme = cs[index];
	//const int pair_idx = pair_arr[index];
	//const int dermis_idx = dermis_arr[index];
	...
	float spring_force_to_memb = 0.0f;
	if (pair_idx >= 0){
...
__device__ float4 al_air_de_interact_impl(const CellPos cme, const CellConnectionData*const conn_ptr,const CellPos*const RESTRICT cs, //rewrite cs
	const CELL_STATE*const RESTRICT cstate, const float*const agek_arr,  const float my_agek ){
	float4 vel = { 0.0f };
	//cell_pos_set cme = cs[index];
	for (int j = 0; j < conn_ptr->connect_num; j++){
		const int opidx = conn_ptr->connect_index[j];
...
__device__ float CI_pair(const CellPos c1,const CellPos c2,const float spr_nat_len) {
	const float inv_dist = rsqrtf(p_cell_dist_sq(c1, c2));
...
__device__ void pair_interact(const CellIndex index,const CellPos*const RESTRICT cs,CellPos*const RESTRICT new_cs,
	const CellIndex pair_index,const float spr_nat_len){
	const CellPos cme = cs[index];
	const CellPos cpair = cs[pair_index];
	const float4 vs = p_diff4(cme, cpair);
	const float coef = CI_pair(cme, cpair, spr_nat_len);
...
__global__ void non_memb_interact(
	...
	const float*const RESTRICT agek_arr,
	const int*const RESTRICT div_time_arr,
	const float*const RESTRICT spr_nat_len_arr){

...
inline double t_sqrtD(double x)
{
double xHalf = 0.5 * x;
		...
		double r[], int indx[])
{
  ...
  double diffx,diffy,diffz,rad_sum;
  int anx, any, anz;
  int aix, aiy, aiz;
	...
	for(l = anz - ii; l <= anz + ii; l++) {
	  ...
	  for(m = 0; m < aindx[aix][aiy][aiz]; m++) {
		...
		continue;
          ...
          diffx=perio_diff_x(xx[i],xx[o]);
          diffy=perio_diff_y(yy[i],yy[o]);
          diffz=zz[i]-zz[o];
          rad_sum=r[i] + r[o];
...
inline double norm_inv(double x,double y ,double z){
    return t_sqrtD(x*x+y*y+z*z);
}
...
return norm_inv(x1-x2,y1-y2,z1-z2);
}
...
/* irx = (int) Rad * NX / LENGTH_X = (int) Rad / dx */
{
  ...
  int nbzmax,bzmax,nbzmin,bzmin,zmax,zmin;
  double mx, my, mz;double fac_r,ox,oy,oz,orad;
  int irz2,irx2,iry2;
     irz2=irz*2;
     iry2=iry*2;
     irx2=irx*2;
...
ox=xx[j];oy=yy[j];oz=zz[j];
	...
	for (l = -iry; l <= iry; l++) {
      ...
      nbzmax=iz+irz;bzmax=NZ-1;
       zmax = bzmax<nbzmax?bzmax:nbzmax;
      nbzmin=iz-irz;bzmin=1;
     zmin= bzmin>nbzmin?bzmin:nbzmin;
      for (m = zmin; m <= zmax; m++) {
        //ipz = imz = m;
        //mz = m * dz;
          map[ipx][ipy][m]=is_within_cell_sq(mx, my, m*dz, ox,oy,oz,orad)?j:-1;
	  }
	}
      }
    ...
    for (k = -irx2; k <= irx2; k++) {
      ...
      for (l = -iry2; l <= iry2; l++) {
        ...
        nbzmax=iz+irz2;bzmax=NZ-1;
         zmax = bzmax<nbzmax?bzmax:nbzmax;
        nbzmin=iz-irz2;bzmin=0;
       zmin= bzmin>nbzmin?bzmin:nbzmin;
        for (m = zmin; m <= zmax; m++) {
          //ipz = imz = m;
          //mz = imz * dz;
        map2[ipx][ipy][m] = is_within_cell_sq(mx, my, m*dz, ox,oy,oz, fac_r)?1:0;
        }
	  }
	}
      }

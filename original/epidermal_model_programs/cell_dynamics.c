...
{
  double ra, rb, distlj, cr_dist, lambda_dist, dum_lj6, dum_ljmain, dumxx, dumyy, dumzz;
  ra=old_r[j]; rb=old_r[l];
  
  distlj = dist3(old_xx[j], old_yy[j], old_zz[j], old_xx[l], old_yy[l], old_zz[l]);
  
  cr_dist = (ra + rb) * P_MEMB;
  lambda_dist = (1.0+P_MEMB)*(ra + rb);
  
  if (distlj < cr_dist) {
    dum_lj6 = pow(cr_dist/distlj, 6);
    dum_ljmain = 4*eps_m * dum_lj6*(dum_lj6-1.0) / (distlj*distlj);
  }
  else if (distlj < ra + rb)	
    dum_ljmain = -(DER_DER_CONST/distlj) * (distlj/cr_dist - 1.0);	
  else { 
    dum_lj6 = pow(cr_dist / (lambda_dist - distlj), 6);
    dum_ljmain = -(DER_DER_CONST/ (ra + rb))*((1.0-P_MEMB)/P_MEMB)

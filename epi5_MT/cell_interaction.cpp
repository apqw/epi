...
#define CIFuncDecl(name) struct name{static double CIFuncCName(const Cell*const& c1,const Cell*const& c2);}
template<class Fn>
inline void cell_interaction_apply(Cell*const& c1, Cell*const& c2) {
...
inline bool is_der_near(const Cell*const& der1, const Cell*const& der2) {
	return sqrt(p_cell_dist_sq(der1, der2))+ cont::delta_R < der1->radius + der2->radius;
}

inline double ljmain_der_near(const Cell*const& der1, const Cell*const& der2) {
	/*
		距離の2乗だけでは不可
	*/
...
inline bool is_near(const Cell*const& c1, const Cell*const& c2) {
	//double rad_sum = c1->radius + c2->radius;
	return p_cell_dist_sq(c1, c2) < (c1->radius + c2->radius)*(c1->radius + c2->radius);
}

inline double ljmain_der_far(const Cell*const& der1, const Cell*const& der2) {
	double dist = sqrt(p_cell_dist_sq(der1, der2));
	double dist2 = dist + cont::delta_R;
	double rad_sum = der1->radius + der2->radius;
	double LJ6 = rad_sum*rad_sum / (dist2*dist2);
...
inline double ljmain(const Cell*const& c1, const  Cell*const& c2) {
...
inline double adhesion(const Cell*const& c1, const  Cell*const& c2,double spring_const) {
...
double CI_der_to_der::CIFuncCName(const Cell*const& c1, const Cell*const& c2) {
...
double CI_al_air_de_to_al_air_de_fix_mu::CIFuncCName(const Cell*const& c1, const Cell*const& c2) {
	if (is_near(c1, c2)) {
		return ljmain(c1, c2);
	}
	else {
...
double CI_fix_mu_to_fix_mu::CIFuncCName(const Cell*const& c1, const Cell*const& c2) {
	//pairかどうかは外で判定しよう
	if (is_near(c1, c2)) {
		return ljmain(c1, c2);
	}
...
double CI_mu_to_memb::CIFuncCName(const Cell*const& c1, const Cell*const& c2) {
...
double CI_fix_to_memb::CIFuncCName(const Cell*const& c1, const  Cell*const& c2) {
	if (is_near(c1, c2)) {
		return ljmain(c1, c2);
	}
	else {
...
double CI_memb_to_memb::CIFuncCName(const Cell*const& c1, const  Cell*const& c2) {
	using namespace cont;
	double rad_sum = c1->radius+ c2->radius;
...
inline double CI_other::CIFuncCName(const Cell*const& c1, const  Cell*const& c2) {
	return ljmain(c1, c2);
}

inline double CI_pair::CIFuncCName(const Cell*const& c1, const  Cell*const& c2) {
	double dist = sqrt(p_cell_dist_sq(c1, c2));
	double force = cont::Kspring_division*(dist - c1->spr_nat_len);
	return -force / dist;
}

inline void wall_interaction(Cell*const& c) {
...
////////////////////////////////////////////////////////////////////
inline void _memb_bend_calc1(Cell *const& memb)
{
...
inline void _memb_bend_calc2(Cell *const& memb)
{
	memb->md.ipn =
...
inline void _memb_bend_calc3(Cell *const& memb)
{
...
inline bool collide_with_wall(const Cell*const& c) {
	return c->z() < c->radius;
}



inline void _MEMB_interaction(Cell*const& memb) {
	if (collide_with_wall(memb)) {
		wall_interaction(memb);
	}
	/*
	memb->connected_cell.foreach([&memb](Cell* conn) {
...
void _DER_interaction(Cell*const& der) {
...
void _AL_AIR_DE_interaction(Cell*const& aad) {
...
void _FIX_interaction(Cell*const& fix) {
...
inline bool paired_with_fix(const Cell*const& c) {
	return c->pair != nullptr&&c->pair->state == FIX;
}
void _MUSUME_interaction(Cell*const& musume) {
...
inline void _pair_interaction(Cell*const& paired) {
